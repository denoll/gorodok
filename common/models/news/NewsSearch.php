...
class NewsSearch extends News
{
	...
	public function search($params)
	{
		...
		if ($this->load($params) || !empty($cat)) {
			$query = News::find()->with('tags')->with('cat')->where('(publish < NOW() AND (unpublish < NOW()OR unpublish IS NULL))')->andWhere(['status' => 1])->orderBy('publish DESC');
		} else {
			$query = News::find()->with('tags')->with('cat')->where('(publish < NOW() AND (unpublish < NOW()OR unpublish IS NULL))')->andWhere(['status' => 1, 'on_main' => 1])->orderBy('publish DESC');
		}
		...
		if (!empty($cat)) {
			NewsCat::setSessionCategoryTree($cat);
			$leaves = NewsCat::getLeavesNodesByAlias($cat);
			$cur_cat = NewsCat::findOne(['alias'=>$cat]);
			$cat_id[] = $cur_cat->id;
			if (!empty($leaves[0])) {
