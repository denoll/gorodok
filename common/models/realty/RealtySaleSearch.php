...
class RealtySaleSearch extends VRealtySale
{
	...
	public function rules()
	{
		return [
			...
			[['main_img', 'base_url'], 'string', 'max' => 50],
	...
	public function search($params)
	{
		...
		$query->andWhere($sql);
		if ($this->resell != '') {
			$query->andWhere(['resell' => $this->resell]);
		}
		if ($this->repair != '') {
			$query->andWhere(['repair' => $this->repair]);
		}
		if ($this->type != '') {
			$query->andWhere(['type' => $this->type]);
		}
		if ($this->elec != '') {
			$query->andWhere(['elec' => $this->elec]);
		}
		if ($this->gas != '') {
			$query->andWhere(['gas' => $this->gas]);
		}
		if ($this->water != '') {
			$query->andWhere(['water' => $this->water]);
		}
		if ($this->heating != '') {
			$query->andWhere(['heating' => $this->heating]);
		}
		if ($this->tel_line != '') {
			$query->andWhere(['tel_line' => $this->tel_line]);
		}
		if ($this->internet != '') {
			$query->andWhere(['internet' => $this->internet]);
		}
