		...
		<div class="container-fluid" style="border: 1px #cfcfcf solid; padding: 1px; 10px; margin: 0px;">
			<div class="col-sm-2 side_left sm-margin-bottom-20">
				<div class="thumbnail" style="padding: 1px; margin: 15px 0px 17px 0px;">
					<?= Html::a(\common\helpers\Thumb::img($model['base_url'],$model['img'], '100%'), ['/konkurs/konkurs/view', 'cat'=> $model['cat']['slug'], 'id' => $model['slug']]) ?>
				</div>
			</div>
			<div class="col-md-10 side_left">
				<h2 style="margin: 5px 0px;">
					Конкурс: <?= Html::a(Html::encode($model['name']), ['/konkurs/konkurs/view', 'cat'=> $model['cat']['slug'], 'id' => $model['slug']]) ?>
				</h2>
				<p>
					<?= $model['title']; ?>
				</p>
				<i class="small-text">Категория: </i>&nbsp;<span style="font-weight: bold;"><?= $model['cat']['name'] ? Html::a($model['cat']['name'], ['/konkurs/konkurs/index', 'cat'=>$model['cat']['slug']]) : 'не указана' ?></span></br>
