					...
					<?php foreach ($model as $item) { ?>
						<div class="inner-results">
							...
							<div class="btn-group" style="margin: 5px 0px;">
								...
								echo '<span ' . $status . ' onclick="changeUp(' . $item['id'] . ')" id="up-btn-' . $item['id'] . '" class="btn-u btn-u-sm btn-block btn-u-orange ads-btn" title="Поднять объявление на верх"><i class="fa fa-arrow-up"></i>&nbsp;&nbsp;Поднять объявление на верх (стоимость: ' . $pay['service_up_pay'] . ' руб.)</span>';
								echo '<span ' . $status . ' onclick="changeVip(' . $item['id'] . ')" id="vip-btn-' . $item['id'] . '" class="btn-u btn-u-sm btn-block btn-u-dark-blue ads-btn" title="Выделить объявление  цветом"><i class="fa fa-trophy"></i>&nbsp;&nbsp;Выделить цветом сроком на ' . $period['vip'] . ' дней. <br>и поднять на верх (стоимость: ' . $pay['service_vip_pay'] . ' руб.)</span>';
