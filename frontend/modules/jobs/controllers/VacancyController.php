	...
	public function actionChangeStatus()
	{
		...
		if ($post) {
			...
			if ($model->save()) {
				...
				CommonQuery::sendChangeStatusEmail($user->getId(), $model, Url::to('@frt_url/jobs/vacancy/my-vacancy'));
			} else {
			...
			if ($user->account >= $pay['vac_up_pay']) {
				...
				if ($model->save() && $u_account->save() && CommonQuery::userAccontUpdateSum($user->id)) {
					...
					$arr = [
						...
						'pay' => $pay['vac_up_pay'],
						'date' => date('Y-m-d'),
						'message' => 'Вакансия поднята на верх. С вашего счета списано ' . $pay['vac_up_pay'] . 'руб.',
						'm_type' => 'success'
					];
					echo json_encode($arr);
				} else {
					$arr = [
						'message' => 'Вакансия не поднята',
						'm_type' => 'danger'
					];
					echo json_encode($arr);
				}
				...
				$arr = [
					'message' => 'Вакансия не поднята. На вашем счёте недостаточно средств.',
					'm_type' => 'danger'
				];
				echo json_encode($arr);
			}
			...
			if ($user->account >= $pay['vac_vip_pay']) {
				...
				if ($model->save() && $u_account->save() && CommonQuery::userAccontUpdateSum($user->id)) {
					...
					$arr = [
						...
						'pay' => $pay['vac_vip_pay'],
						'date' => date('Y-m-d'),
						'message' => 'Вакансия выделена на ( ' . $period['vip'] . ' )дней и поднято на верх. С вашего счета списано ' . $pay['vac_vip_pay'] . 'руб.',
						'm_type' => 'success'
					];
					echo json_encode($arr);
				} else {
					$arr = [
						'message' => 'Вакансия не выделена и не поднята.',
						'm_type' => 'danger'
					];
					echo json_encode($arr);
				}
				...
				$arr = [
					'message' => 'Вакансия не выделена и не поднята. На вашем счёте недостаточно средств.',
					'm_type' => 'danger'
				];
				echo json_encode($arr);
			}
