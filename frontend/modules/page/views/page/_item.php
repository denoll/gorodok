        ...
        <div class="container-fluid" style="border: 1px solid #D9D9D9; padding: 1px; 10px; margin: 0px;">
            <div class="col-sm-3 side_left sm-margin-bottom-20">
                <div class="thumbnail" style="padding: 1px; margin: 15px 0px 17px 0px;">
                    <?= Html::a(Avatar::imgPage($model['thumbnail'], '100%'), [Url::to('/page/page/view'),'cat'=>$model['cat']['alias'], 'id' => $model['alias']]) ?>
                </div>
            </div>
            <div class="col-md-9">

                <h2 style="font-size: 1.25em; margin: 3px 0px;">
                    <?= Html::a(Html::encode($model['title']), [Url::to('/page/page/view'),'cat'=>$model['cat']['alias'], 'id' => $model['alias']]) ?>
                </h2>
                <p style="margin: 2px 0;"><i class="small-text">Дата:&nbsp;<?= \Yii::$app->formatter->asDate($model['modifyed_at'], 'long') ?></i>
                <span style="padding-left: 15px;"><i class="small-text">Категория:&nbsp;&nbsp;</i><strong><?=Html::a($model['cat']['name'],['/page/page/index','cat'=>$model['cat']['alias']])?></strong></span>
                </p>
