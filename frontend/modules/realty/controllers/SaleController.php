...
class SaleController extends Controller
{
	...
	public function actionDelete($id)
	{
		...
		$images = RealtySaleImg::find()->where([ 'id_ads' => $id ])->all();
		$path = Url::to('@frt_dir/img/realty_sale/');
		if ( $images ) {
	...
	public function actionChangeStatus()
	{
		...
		if ($post) {
			...
			if ($model->save()) {
				...
				CommonQuery::sendChangeStatusEmail($user->getId(), $model, Url::to('@frt_url/realty/sale/my-ads'));
			} else {
	...
	public function actionChangeVip()
	{
		...
		if ($post) {
			...
			if ($user->account >= $pay['realty_vip_pay']) {
				...
				$u_account->date = new Expression('NOW()');
				$u_account->description = 'Выделение объявления о продаже недвижимости №' . $model->id . '. На срок: ' . $period['vip'] . 'дней.';
