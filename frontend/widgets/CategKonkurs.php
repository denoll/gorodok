...
class CategKonkurs extends Widget
{
	public $level;
	...
	public function run()
	{
		...
		echo Html::a('<i class="fa fa-konkurspaper-o"></i>&nbsp;&nbsp;Все конкурсы', ['/konkurs/konkurs/index'], ['style' => 'color:#fff; text-decoration:none; font-size: 0.9em;']);
		echo '</h3>';
		echo '</div>';
		if(!Yii::$app->user->isGuest && !empty($id_konkurs)){
			if(KonkursItem::verificationsUserItems($id_konkurs)){
				$konkurs = Konkurs::findOne($id_konkurs);
				echo Html::a('<i class="fa fa-plus"></i>&nbsp;&nbsp;Изменить мои элементы', ['/konkurs/konkurs/my-items','id'=>$konkurs->slug],
					['class' => 'btn-u btn-u-orange cat-button', 'style' => 'padding: 5px 7px 5px 7px; text-align:center; font-size:15px; width:100%;']
				);
			}else{
				echo Html::a('<i class="fa fa-plus"></i>&nbsp;&nbsp;Принять участие в конкурсе', ['/konkurs/item/create'],
					['class' => 'btn-u btn-u-orange cat-button', 'style' => 'padding: 5px 7px 5px 7px; text-align:center; font-size:15px; width:100%;']
				);
			}
		...
		echo '<i class="fa fa-bars">&nbsp;&nbsp;Все конкурсы </i>';
		echo '</a>';
		echo '</div>';
	...
	{
		return KonkursCat::getDb()->cache(function ($db) {
			return KonkursCat::find()
				->where(['status' => KonkursCat::STATUS_ACTIVE])
				->asArray()
				->orderBy('order')
				->all();
		}, Arrays::CASH_TIME);
	...
	private function tree($cat)
	{
		...
		$path = Url::to('/konkurs/konkurs/index');
		echo '<ul class="nav navbar-nav">';
		foreach ($cat as $node) {
			if ($node['lvl'] == 0) {
				echo '<li class="dropdown">';
				if ($node['slug'] == $activ_el) {
					echo Html::a($node['name'] . '<b class="dropme"></b>', [$path, 'cat' => $node['slug']], ['data-toggle' => 'dropdown', 'class' => 'dropdown-toggle active']);
				} else {
					echo Html::a($node['name'] . '<b class="dropme"></b>', [$path, 'cat' => $node['slug']], ['data-toggle' => 'dropdown', 'class' => 'dropdown-toggle']);
				}
				echo '</li>';
			}
		}
